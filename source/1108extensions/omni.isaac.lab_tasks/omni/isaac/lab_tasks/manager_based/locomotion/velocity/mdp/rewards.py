...
def height_penalty_toolow(env, min_height: float = 0.25, weight: float = -2.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def diagonal_gait_reward(env: ManagerBasedRLEnv, command_name: str, sensor_cfg: SceneEntityCfg, threshold: float
    ...
    left_front = first_contact[:, 0]
    right_front = first_contact[:, 1]
    left_hind = first_contact[:, 2]
    right_hind = first_contact[:, 3]
